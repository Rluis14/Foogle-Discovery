...
user_router.post("/favorites",verifyToken,async(req:any,res)=>{
    try{
        ...
        await userRef.update({
            saved_recipe_ids: admin.firestore.FieldValue.arrayUnion(recipe_id),
        });
...
user_router.delete("/favorites", verifyToken, async (req: any, res) => {
    try {
        const userId = (req as any).user.uid;
        const userRef = db.collection("User").doc(userId);
        const userDoc = await userRef.get();
        ...
        await userRef.update({
            saved_recipe_ids: admin.firestore.FieldValue.arrayRemove(recipe_id),
        });
